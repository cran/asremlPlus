...
{ #Check options
  ...
  { barplot <- TRUE
    ...
    { pred.plot <-  pred.plot + 
      ...
      if (scheme.opt == "black")
        pred.plot <-  pred.plot + layer(geom = "bar", stat="identity", fill="grey50") 
      else
        pred.plot <-  pred.plot + layer(geom = "bar", stat="identity", fill=cbPalette[1])
      if (intervals)
      { if (scheme.opt == "black")
  ...
  { if (intervals)
    ...
    { pred.plot <-  pred.plot + 
      ...
      if (scheme.opt == "black")
        pred.plot <-  pred.plot + layer(geom = "line", colour = "black") 
      else
        pred.plot <-  pred.plot + layer(geom = "line", colour = cbPalette[1])
      if (intervals)
      { if (scheme.opt == "black")
          pred.plot <-  pred.plot + geom_errorbar(aes_string(ymin=ylow, ymax=yupp),  
