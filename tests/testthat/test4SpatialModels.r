...
test_that("Wheat_spatial_models_asreml4", {
  ...
  testthat::expect_true(all(abs(spatial.asrts$spatial.IC$AIC - 
                                  c(1720.891, 1653.094, 1639.792, 1644.007, 1653.110)) < 0.10))
  ...
  testthat::expect_true(all.equal(spatial.asrts$spatial.IC[-1,], infoEach[1:4,-3], 
                                  tolerance = 1e-05))
  testthat::expect_true(all.equal(infoEach$AIC[rownames(infoEach) == 'TPP1LS'],1653.110, 
                                  tolerance = 1e-05))
  ...
  testthat::expect_true(all(rownames(spatial.asrts$spatial.IC) == 
                              c("nonspatial", "corr", "TPNCSS", "TPPCS", "TPP1LS")))
  testthat::expect_true(all(abs(spatial.asrts$spatial.IC$AIC - 
                                  c(1690.964, 1653.978, 1639.792, 1644.007, 1653.110)) < 0.10))
...
cat("#### Test for wheat76 corr spatial models with asreml4\n")
test_that("Wheat_corr_models_asreml4", {
  skip_if_not_installed("asreml")
  skip_on_cran()
  library(dae)
  library(asreml)
  ...
  testthat::expect_true(all.equal(infoEach$AIC, c(1722.815, 1653.110), tolerance = 1e-05))
...
cat("#### Test hetero variances for HEB25 with asreml4\n")
test_that("HEB25_heterovar_asreml4", {
  skip_if_not_installed("asreml")
  skip_on_cran()
  library(dae)
  library(asreml)
  ...
  testthat::expect_true(all(abs(HEB25.spatialLP.asrts$spatial.IC$AIC - 
                                  c(524.1825, 480.9052, 477.0927, 471.3009, 476.9187) < 1e-03)))
  ...
  testthat::expect_equal(summ.ds$bound, c("P","U","U","P","U","P",
                                       "P","P","P","P","P","P"))
  #Two components are missing fromdsum
